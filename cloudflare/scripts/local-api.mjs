...
import { buildTourAssignEmail, sendResendEmail } from '../src/auth.js'
import {
  openAclDb,
  normalizeEmail,
  newGroupId,
...
async function handleAdmin(req, res, user, pathname, url) {
  ...
  const grantsMatch = pathname.match(/^\/api\/admin\/projects\/(.+)\/grants$/)
  if (grantsMatch && req.method === 'PUT') {
    ...
    for (const g of grants) {
      ...
      let principal_id =
        principal_type === 'user'
          ? normalizeEmail(g?.principal_id)
          : String(g?.principal_id || '').trim()
    ...
    const stmts = [aclDb.prepare('DELETE FROM project_grants WHERE cloud_key = ?').bind(key)]
    ...
    await aclDb.batch(stmts)
    return json(res, { ok: true, key, grants: finalGrants })
  }

  // POST /api/admin/notify — email selected users (Resend if configured; else stub)
  if (pathname === '/api/admin/notify' && req.method === 'POST') {
    ...
    if (!unique.length) return json(res, { error: 'Select at least one person' }, 400)
    ...
    const viewerUrl = `http://${HOST}:${PORT}/`

    const recipients = []
    for (const email of unique) {
      const row = await aclDb
        .prepare('SELECT email, display_name FROM users WHERE email = ? COLLATE NOCASE')
        .bind(email)
        .first()
      if (row) recipients.push(row)
    }
    if (!recipients.length) return json(res, { error: 'No matching allowlisted users' }, 400)

    const env = {
      RESEND_API_KEY: process.env.RESEND_API_KEY,
      RESEND_FROM: process.env.RESEND_FROM,
    }
    const canSend = !!(env.RESEND_API_KEY && env.RESEND_FROM)
    const sent = []
    const failed = []
    for (const row of recipients) {
      ...
      if (!canSend) {
        console.log('[local-api notify stub]', row.email, content.subject)
        sent.push(row.email)
        continue
      }
